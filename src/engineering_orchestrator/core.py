...
"""
    ...
    def _agent_context_pack_registry_catalog_context(self, task: HarnessTask, command: AcceptanceCommand) -> dict[str, Any]:
        ...
        known_candidates = [
            ("src/engineering_orchestrator/executors.py", "executor registry and agent adapter catalog"),
            ("src/engineering_orchestrator/profiles.py", "profile and starter roadmap catalog"),
            ("src/engineering_orchestrator/goal_planner.py", "goal-to-roadmap task catalog"),
            ("src/engineering_orchestrator/domain_frontend.py", "domain frontend catalog"),
    ...
    def _command_is_user_experience_gate(self, command: AcceptanceCommand) -> bool:
        ...
        return (
            "engineering_orchestrator.browser_e2e" in command_text
            or "engineering_harness.browser_e2e" in command_text
        )
    ...
    def _run_is_user_experience_gate(self, task: HarnessTask, run: CommandRun) -> bool:
        ...
        if isinstance(gate, dict) and str(gate.get("kind") or "").strip() == BROWSER_USER_EXPERIENCE_GATE_KIND:
            return True
        command_text = str(run.command or "")
        return (
            "engineering_orchestrator.browser_e2e" in command_text
            or "engineering_harness.browser_e2e" in command_text
        )
